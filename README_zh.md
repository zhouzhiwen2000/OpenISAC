...
| `profiling_modules` | `string` | `""` | 性能统计模块列表，逗号分隔。常用值包括 `modulation`、`latency`、`ldpc_encode`、`sensing_proc`、`agc` 和 `uplink`；`all` 表示全部。BS 端到端时延统计只有在同时包含 `modulation` 和 `latency` 时才启用。 |
