...
| `measurement_enable` | `bool` | `false` | 启用 CPU 版内部测量模式。启用后，`OFDMModulator` 不再监听 `udp_input_*`，而是内部生成确定性的 PRBS 载荷；`OFDMDemodulator` 会把测量载荷转入 BER/BLER/EVM 统计。CPU 二进制会在本地处理该模式。 |
...
| `control_port` | `int` | `10000` | 双向控制通道的 ZeroMQ ROUTER 绑定端口。 |
