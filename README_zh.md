...
| `udp_egress_pacer_queue_packets` | `int` | `512` | egress pacer 最多缓存的 UDP 数据报数量，超过后丢弃最旧数据报。 |
| `udp_egress_pacer_max_delay_ms` | `float` | `250` | egress pacer 中数据报允许排队的最长时间；设为 `0` 可关闭按年龄丢包。 |
...
| `profiling_modules` | `string` | `""` | 性能统计模块列表，逗号分隔。常用值包括 `modulation`、`latency`、`data_ingest`、`sensing_proc`；`all` 表示全部。BS 端到端时延统计只有在同时包含 `modulation` 和 `latency` 时才启用。 |
| `downlink_cpu_cores` | `int[]` | `[]` | BS 下行 CPU 核列表：索引 `0`、`1`、`2` 分别绑定 TX、调制和数据输入线程。 |
