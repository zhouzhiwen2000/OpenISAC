...
| `ertm_to_enable` | `bool` | `false` | 开启 CPU 路径 eRTM TO 估计。BS 将最新上行 delay spectrum 和运行时 `DUTI` 封装到内部下行 LDPC payload；UE 消费该 payload 并打印 TO 估计，不转发到用户 UDP。 |
| `ertm_dl_rf_delay_s` | `float` / 秒 | `0.0` | eRTM TO 方程使用的下行 RF 链路校准延迟项。 |
| `ertm_ul_rf_delay_s` | `float` / 秒 | `0.0` | eRTM TO 方程使用的上行 RF 链路校准延迟项。 |
...
| `profiling_modules` | `string` | `""` | 性能统计模块列表，逗号分隔。常用值包括 `modulation`、`latency`、`ldpc_encode`、`sensing_proc`；`all` 表示全部。BS 端到端时延统计只有在同时包含 `modulation` 和 `latency` 时才启用。 |
...
| `ertm_to_enable` | `bool` | `false` | 开启 CPU 路径 eRTM TO payload 消费和 TO 日志。 |
| `ertm_dl_rf_delay_s` | `float` / 秒 | `0.0` | eRTM TO 方程使用的下行 RF 链路校准延迟项。 |
| `ertm_ul_rf_delay_s` | `float` / 秒 | `0.0` | eRTM TO 方程使用的上行 RF 链路校准延迟项。 |
...
| `profiling_modules` | `string` | `""` | 性能统计模块列表，逗号分隔。常用值包括 `demodulation`、`cfo`、`sync`、`agc`、`align`、`snr`、`uplink`；`all` 表示全部。`cfo` 控制 CUDA CFO 诊断日志，`sync` 控制逐 alias 同步峰值比较日志，`agc` 控制 AGC 日志，`align` 控制运行时 `ALGN:` 日志，`snr` 会周期性打印当前 UE 接收路径的 `_snr_db / _noise_var / _llr_scale`，`uplink` 控制 `[UL-TX]` timing/waveform 诊断日志。 |
