import numpy as np
import os
import faulthandler

# ====== Backend Detection ======
# Priority: NVIDIA GPU (CuPy) > Intel GPU (dpnp) > CPU with Numba > Pure NumPy

USE_NVIDIA_GPU = False
USE_INTEL_GPU = False
cp = None  # CuPy or dpnp array module
xp = np    # Active array module (defaults to NumPy)

# 1. Try NVIDIA GPU via CuPy
try:
    import cupy as _cp  # type: ignore
    try:
        _gpu_count = _cp.cuda.runtime.getDeviceCount()
        if _gpu_count > 0:
            USE_NVIDIA_GPU = True
            cp = _cp
            xp = _cp
            print(f"Backend: Using NVIDIA GPU via CuPy (devices: {_gpu_count})")
    except Exception as _e:
        print(f"Backend: CuPy available but CUDA init failed: {_e}")
except ImportError:
    pass

# 2. Try Intel GPU via dpnp (oneAPI) - with timeout protection
if not USE_NVIDIA_GPU:
    def _try_intel_gpu():
        """Attempt Intel GPU initialization with timeout protection."""
        global USE_INTEL_GPU, cp, xp
        try:
            import dpnp as _dpnp  # type: ignore
            import dpctl  # type: ignore
            
            # Check for Intel GPU devices
            try:
                # Try newer dpctl API first
                _intel_devices = [d for d in dpctl.get_devices() if d.is_gpu]
            except:
                # Fallback for older dpctl
                _intel_devices = []
            
            if _intel_devices:
                _intel_gpu = _intel_devices[0]
                # Create a queue for the GPU device - this is how dpnp knows which device to use
                try:
                    # dpctl >= 0.14 style
                    _queue = dpctl.SyclQueue(_intel_gpu)
                    # dpnp will use this queue when we pass device argument
                except:
                    pass  # dpnp may still work with default device
                return _dpnp, _intel_gpu.name
            else:
                print("Backend: dpnp available but no Intel GPU detected")
                return None, None
        except ImportError:
            print("Backend: dpnp not available (install with: pip install dpnp dpctl)")
            return None, None
        except Exception as _e:
            print(f"Backend: dpnp init failed: {_e}")
            return None, None
    
    # Run with timeout to prevent freezing
    import threading
    _intel_result = [None, None]
    def _intel_init_thread():
        _intel_result[0], _intel_result[1] = _try_intel_gpu()
    
    _init_thread = threading.Thread(target=_intel_init_thread, daemon=True)
    _init_thread.start()
    _init_thread.join(timeout=5.0)  # 5 second timeout
    
    if _init_thread.is_alive():
        print("Backend: Intel GPU detection timed out (driver issue?); using CPU")
    elif _intel_result[0] is not None:
        USE_INTEL_GPU = True
        cp = _intel_result[0]
        xp = _intel_result[0]
        print(f"Backend: Using Intel GPU via dpnp ({_intel_result[1]})")

# Combined GPU flag for code paths
USE_GPU = USE_NVIDIA_GPU or USE_INTEL_GPU

if not USE_GPU:
    print("Backend: No GPU acceleration available; using CPU")

# 3. Try Numba for CPU acceleration (used when no GPU)
HAVE_NUMBA = False
try:
    from numba import njit, prange
    HAVE_NUMBA = True
    print("Backend: Numba available for CPU acceleration")
except ImportError:
    print("Backend: Numba not available")

# Helper function to convert GPU arrays to NumPy (works with CuPy and dpnp)
def to_numpy(arr):
    """Convert array to NumPy, handling both CuPy and dpnp arrays."""
    if hasattr(arr, 'get'):  # CuPy style
        return arr.get()
    elif hasattr(arr, 'asnumpy'):  # dpnp style (also works for CuPy)
        return arr.asnumpy()
    elif hasattr(arr, '__array__'):
        return np.asarray(arr)
    return arr
import socket
import struct
import matplotlib.pyplot as plt
import matplotlib.cm as cm
from matplotlib.animation import FuncAnimation
from matplotlib.widgets import Button, TextBox, Slider
import matplotlib.patches as patches
import scipy.io as sio
import datetime
try:
    from scipy.signal import stft  # type: ignore
    _SCIPY_AVAILABLE = True
except Exception:
    _SCIPY_AVAILABLE = False
    def stft(x, fs=1.0, window='hamming', nperseg=256, noverlap=192, nfft=256, return_onesided=False):
        """Minimal STFT fallback using NumPy if SciPy is unavailable."""
        x = np.asarray(x)
        if x.ndim != 1:
            x = x.ravel()
        step = nperseg - noverlap
        if step <= 0:
            raise ValueError("noverlap must be < nperseg")
        if isinstance(window, str):
            if window.lower() == 'hamming':
                w = np.hamming(nperseg).astype(np.float32)
            else:
                w = np.ones(nperseg, dtype=np.float32)
        else:
            w = np.asarray(window).astype(np.float32)
        n_frames = 1 + max(0, (len(x) - nperseg) // step)
        if n_frames <= 0:
            return np.array([]), np.array([]), np.zeros((0, 0), dtype=np.complex64)
        Zxx = np.empty((nfft, n_frames), dtype=np.complex64)
        for i in range(n_frames):
            start = i * step
            seg = x[start:start + nperseg]
            if len(seg) < nperseg:
                seg = np.pad(seg, (0, nperseg - len(seg)))
            segw = (seg * w).astype(np.complex64)
            buf = np.zeros(nfft, dtype=np.complex64)
            buf[:nperseg] = segw
            Zxx[:, i] = np.fft.fft(buf)
        f = np.fft.fftfreq(nfft, d=1.0 / fs)
        t = (np.arange(n_frames) * step) / fs
        if return_onesided:
            keep = f >= 0
            return f[keep], t, Zxx[keep, :]
        return f, t, Zxx
import sys
import time
import threading
from queue import Queue, Empty
from collections import deque
import argparse

# Native-extension crashes (e.g., GUI/CUDA stack) may not raise Python exceptions.
# Enable low-level traceback dump for post-mortem diagnosis.
try:
    # Use stderr so no log file is created during normal runs.
    faulthandler.enable(all_threads=True)
except Exception:
    pass

# ====== Global Configuration ======
UDP_IP = "0.0.0.0"
DEFAULT_CONTROL_PORT = 9999  # Default control port before heartbeat update
FFT_SIZE = 1024
NUM_SYMBOLS = 100
MAX_CHUNK_SIZE = 60000
HEADER_SIZE = 12  # 3*uint32 (frame_id, total_chunks, chunk_id)
MAX_QUEUE_SIZE = 5  # Pre-render frame queue size (prevent memory overflow)
SOCKET_BUFFER_SIZE = 8 * 1024 * 1024  # 8MB UDP buffer, prevent packet loss
MAX_RANGE_BIN = 1000  # Max displayed range bin
MAX_DOPPLER_BINS = 1000  # Max displayed Doppler bin

# Zero padding parameters
RANGE_FFT_SIZE = 10240   # Points after zero padding in range dimension
DOPPLER_FFT_SIZE = 1000  # Points after zero padding in Doppler dimension
enable_mti = True  # Enable MTI (Moving Target Indication)
enable_range_window = True  # Enable windowing in range (time delay) dimension
enable_doppler_window = True  # Enable windowing in doppler dimension

# ====== Thread-safe Data Structures ======
RAW_QUEUE_SIZE = 20
DISPLAY_QUEUE_SIZE = 5
DISPLAY_DOWNSAMPLE = 2 # Downsample factor for display (2 = 4x fewer pixels)

BUFFER_LENGTH = 5000  # Accumulate 5000 time points
selected_range_bin = 0  # Default selected range bin
show_micro_doppler = True  # Whether to show micro-Doppler spectrum
active_channel = 0
running = True  # Animation running status

def parse_args():
    parser = argparse.ArgumentParser(description="Monostatic sensing viewer (multi-channel)")
    parser.add_argument(
        "--ports",
        type=str,
        default="8888,8889",
        help="Comma-separated UDP ports for sensing channels, e.g. 8888,8890",
    )
    parser.add_argument(
        "--control-port",
        type=int,
        default=DEFAULT_CONTROL_PORT,
        help="Fallback control port before heartbeat detection",
    )
    return parser.parse_args()


class ChannelRuntime:
    def __init__(self, ch_id, udp_port, control_port):
        self.ch_id = ch_id
        self.udp_port = udp_port
        self.control_port = control_port

        self.frame_buffer = None
        self.buffer_lock = threading.Lock()

        self.raw_frame_queue = Queue(maxsize=RAW_QUEUE_SIZE)
        self.display_queue = Queue(maxsize=DISPLAY_QUEUE_SIZE)

        self.sender_ip = None
        self.sender_lock = threading.Lock()

        self.range_time_data = None
        self.range_time_lock = threading.Lock()
        self.micro_doppler_buffer = deque(maxlen=BUFFER_LENGTH)
        self.micro_lock = threading.Lock()
        self.gpu_stream = cp.cuda.Stream(non_blocking=True) if USE_NVIDIA_GPU else None

        self.current_display_data = {}
        self.last_frame_id = None


args = parse_args()
UDP_PORTS = [int(x.strip()) for x in args.ports.split(",") if x.strip()]
if not UDP_PORTS:
    UDP_PORTS = [8888, 8889]

CHANNELS = [ChannelRuntime(idx, p, args.control_port) for idx, p in enumerate(UDP_PORTS)]


class FrameBuffer:
    def __init__(self):
        self.frame_id = 0
        self.total_chunks = 0
        self.buffer = [None] * 1024  # Initial size
        self.received_chunks = 0
    
    def init(self, frame_id, total_chunks):
        self.frame_id = frame_id
        self.total_chunks = total_chunks
        self.buffer = [None] * total_chunks
        self.received_chunks = 0
    
    def add_chunk(self, chunk_id, data):
        if chunk_id < self.total_chunks and self.buffer[chunk_id] is None:
            self.buffer[chunk_id] = data
            self.received_chunks += 1
            return self.received_chunks == self.total_chunks
        return False
    
    def assemble_frame(self):
        byte_data = b''.join(self.buffer[:self.total_chunks])
        complex_array = np.frombuffer(byte_data, dtype=np.complex64)
        return self.frame_id, complex_array.reshape((NUM_SYMBOLS, FFT_SIZE))

for _ch in CHANNELS:
    _ch.frame_buffer = FrameBuffer()


def _get_detected_targets(target_ch_id=None):
    targets = []
    if target_ch_id is None:
        ch_list = CHANNELS
    else:
        if target_ch_id < 0 or target_ch_id >= len(CHANNELS):
            return targets
        ch_list = [CHANNELS[target_ch_id]]
    for ch in ch_list:
        with ch.sender_lock:
            if ch.sender_ip is not None:
                targets.append((ch, ch.sender_ip, ch.control_port))
    return targets


def send_control_command(cmd_id, value, target_ch_id=None):
    targets = _get_detected_targets(target_ch_id)
    if not targets:
        print("Error: Cannot send command - no sender detected yet.")
        return
    for ch, ip, cport in targets:
        try:
            sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            command_data = struct.pack("!4s4si", b"CMD ", cmd_id, int(value))
            sock.sendto(command_data, (ip, cport))
            sock.close()
            print(f"Sent {cmd_id.decode(errors='ignore').strip()}={value} to CH{ch.ch_id} {ip}:{cport}")
        except Exception as e:
            print(f"Failed to send command to CH{ch.ch_id}: {str(e)}")


# Send SKIP command
def send_skip_command():
    for _ in range(100):
        if _get_detected_targets(None):
            break
        time.sleep(0.1)
    send_control_command(b"SKIP", 1, None)


# ====== Independent Network Receiver Thread ======
def udp_receiver(ch):
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    try:
        sock.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, SOCKET_BUFFER_SIZE)
    except Exception:
        pass

    try:
        sock.bind((UDP_IP, ch.udp_port))
        print(f"[CH{ch.ch_id}] Listening on UDP port {ch.udp_port}")
    except Exception as e:
        print(f"[CH{ch.ch_id}] Socket bind error: {e}")
        return

    last_log_time = time.time()
    processed_frames = 0

    while running:
        try:
            data, addr = sock.recvfrom(MAX_CHUNK_SIZE + HEADER_SIZE)

            with ch.sender_lock:
                if ch.sender_ip is None:
                    ch.sender_ip = addr[0]
                    print(f"[CH{ch.ch_id}] Detected data sender IP: {ch.sender_ip}")

            # Control heartbeat
            if len(data) >= 8 and data[:4] == b"CTRL":
                with ch.sender_lock:
                    if ch.control_port != addr[1]:
                        ch.control_port = addr[1]
                        print(f"[CH{ch.ch_id}] Control port updated: {ch.sender_ip}:{ch.control_port}")
                continue

            if len(data) < HEADER_SIZE:
                continue

            try:
                frame_id, total_chunks, chunk_id = struct.unpack("!III", data[:HEADER_SIZE])
            except struct.error:
                continue

            chunk_data = data[HEADER_SIZE:]
            with ch.buffer_lock:
                if ch.frame_buffer.frame_id != frame_id:
                    ch.frame_buffer.init(frame_id, total_chunks)
                if ch.frame_buffer.add_chunk(chunk_id, chunk_data):
                    processed_frames += 1
                    current_time = time.time()
                    if current_time - last_log_time >= 1.0:
                        fps = processed_frames / (current_time - last_log_time)
                        print(f"[CH{ch.ch_id}] RX FPS: {fps:.1f} | Frame {frame_id} complete")
                        last_log_time = current_time
                        processed_frames = 0

                    frame_id_done, frame_data = ch.frame_buffer.assemble_frame()
                    if ch.raw_frame_queue.full():
                        try:
                            ch.raw_frame_queue.get_nowait()
                        except Exception:
                            pass
                    ch.raw_frame_queue.put((frame_id_done, frame_data))
        except socket.error as e:
            if getattr(e, "errno", None) != 10054:
                print(f"[CH{ch.ch_id}] Socket error: {e}")
        except Exception as e:
            print(f"[CH{ch.ch_id}] Unexpected receiver error: {str(e)}")

    sock.close()


# ====== Start Receiver Threads ======
receiver_threads = []
for _ch in CHANNELS:
    _t = threading.Thread(target=udp_receiver, args=(_ch,), daemon=True)
    _t.start()
    receiver_threads.append(_t)

# ====== FFT Processing Function ======
if USE_GPU:
    range_window = cp.array(np.hamming(FFT_SIZE).reshape(1, FFT_SIZE), dtype=cp.float32)
    doppler_window = cp.array(np.hamming(NUM_SYMBOLS).reshape(NUM_SYMBOLS, 1), dtype=cp.float32)
else:
    range_window = np.hamming(FFT_SIZE).reshape(1, FFT_SIZE).astype(np.float32)
    doppler_window = np.hamming(NUM_SYMBOLS).reshape(NUM_SYMBOLS, 1).astype(np.float32)

if HAVE_NUMBA:
    @njit(parallel=True, fastmath=True)
    def cpu_prep_range_fft(frame_data, window, fft_size, range_fft_size):
        # frame_data: (NUM_SYMBOLS, FFT_SIZE)
        # Output: padded_data (NUM_SYMBOLS, RANGE_FFT_SIZE)
        
        num_symbols = frame_data.shape[0]
        half_n = fft_size // 2
        
        out = np.zeros((num_symbols, range_fft_size), dtype=np.complex64)
        
        # Flattened window for faster access if possible, or just index nicely
        # window is (1, FFT_SIZE)
        
        for i in prange(num_symbols):
            # FFT shift axis 1 + Window + Pad
            
            # Destination 0..half_n-1 gets Source half_n..FFT_SIZE-1
            for j in range(half_n):
                val = frame_data[i, j + half_n]
                out[i, j] = val * window[0, j]
            
            # Destination half_n..FFT_SIZE-1 gets Source 0..half_n-1
            for j in range(half_n):
                val = frame_data[i, j]
                out[i, j + half_n] = val * window[0, j + half_n]
                
        return out

    @njit(parallel=True, fastmath=True)
    def cpu_prep_doppler_fft(range_time, window, num_symbols, doppler_fft_size, range_fft_size):
        out = np.zeros((doppler_fft_size, range_fft_size), dtype=np.complex64)
        
        for i in prange(num_symbols):
            win_val = window[i, 0]
            for j in range(range_fft_size):
                out[i, j] = range_time[i, j] * win_val
                
        return out

    @njit(parallel=True, fastmath=True)
    def cpu_calc_mag_db(doppler_fft_data, num_symbols, fft_size):
        # doppler_fft_data: (DOPPLER_FFT_SIZE, RANGE_FFT_SIZE)
        rows = doppler_fft_data.shape[0]
        cols = doppler_fft_data.shape[1]
        half_rows = rows // 2
        
        out = np.empty((rows, cols), dtype=np.float32)
        norm_factor = 1.0 / np.sqrt(num_symbols * fft_size)
        
        for j in prange(cols):
            # FFT shift axis 0 and Mag dB
            
            # Dest 0..half_rows-1 gets Source half_rows..rows-1
            for i in range(half_rows):
                val = doppler_fft_data[i + half_rows, j]
                mag = np.abs(val) * norm_factor
                out[i, j] = 20.0 * np.log10(mag + 1e-12)
                
            # Dest half_rows..rows-1 gets Source 0..half_rows-1
            for i in range(half_rows):
                val = doppler_fft_data[i, j]
                mag = np.abs(val) * norm_factor
                out[i + half_rows, j] = 20.0 * np.log10(mag + 1e-12)
                
        return out

def process_range_doppler(frame_data, max_view_range_bins=None, gpu_stream=None):
    """
    Range and Doppler FFT processing (Automatically select GPU/CUDA or CPU/NumPy)
    """
    # Use global MAX_RANGE_BIN if not specified
    if max_view_range_bins is None:
        max_view_range_bins = MAX_RANGE_BIN

    # Select range window based on enable_range_window flag
    if enable_range_window:
        if USE_GPU:
            range_win = range_window
        else:
            range_win = range_window
    else:
        if USE_GPU:
            range_win = cp.ones((1, FFT_SIZE), dtype=cp.float32)
        else:
            range_win = np.ones((1, FFT_SIZE), dtype=np.float32)

    # Select doppler window based on enable_doppler_window flag
    if enable_doppler_window:
        if USE_GPU:
            doppler_win = doppler_window
        else:
            doppler_win = doppler_window
    else:
        if USE_GPU:
            doppler_win = cp.ones((NUM_SYMBOLS, 1), dtype=cp.float32)
        else:
            doppler_win = np.ones((NUM_SYMBOLS, 1), dtype=np.float32)

    if USE_GPU:
        if USE_NVIDIA_GPU and gpu_stream is not None:
            stream_ctx = gpu_stream
        else:
            stream_ctx = None

        if stream_ctx is not None:
            stream_ctx.__enter__()
        try:
            # Transfer input data to GPU
            frame_data_gpu = cp.array(frame_data)

            # 1. Range dimension processing (GPU)
            shifted_data = cp.fft.fftshift(frame_data_gpu, axes=1)
            windowed_data = shifted_data * range_win
            padded_data = cp.zeros((NUM_SYMBOLS, RANGE_FFT_SIZE), dtype=cp.complex64)
            padded_data[:, :FFT_SIZE] = windowed_data
            range_time = cp.fft.ifft(padded_data, axis=1) * RANGE_FFT_SIZE

            # OPTIMIZATION: Crop range bins EARLY (before Doppler processing)
            # We only process the range bins we actually want to display/analyze
            if max_view_range_bins < RANGE_FFT_SIZE:
                 range_time_view = range_time[:, :max_view_range_bins]
            else:
                 range_time_view = range_time

            range_time_cpu = to_numpy(range_time_view)

            # 2. Doppler dimension processing (GPU on reduced data)
            range_time_gpu = cp.array(range_time_cpu, dtype=cp.complex64)
            # Note: doppler_win must match dimension 0 (NUM_SYMBOLS), which is unchanged
            doppler_windowed = range_time_gpu * doppler_win
            
            # Padded doppler size depends on cropped range bins
            view_width = range_time_view.shape[1]
            padded_doppler = cp.zeros((DOPPLER_FFT_SIZE, view_width), dtype=cp.complex64)
            padded_doppler[:NUM_SYMBOLS, :] = doppler_windowed
            
            doppler_fft = cp.fft.fft(padded_doppler, axis=0)
            doppler_shifted = cp.fft.fftshift(doppler_fft, axes=0)
            # Use np.sqrt for scalar to ensure dpnp compatibility
            norm_factor = np.sqrt(NUM_SYMBOLS * FFT_SIZE)
            magnitude = cp.abs(doppler_shifted) / norm_factor
            magnitude_db = 20 * cp.log10(magnitude + 1e-12)
        finally:
            if stream_ctx is not None:
                stream_ctx.__exit__(None, None, None)
                stream_ctx.synchronize()

        return to_numpy(magnitude_db), range_time_cpu, to_numpy(doppler_shifted)

    elif HAVE_NUMBA:
        # Optimized CPU path with Numba
        
        # 1. Range dimension prep (Shift -> Window -> Pad)
        padded_data = cpu_prep_range_fft(frame_data, range_win, FFT_SIZE, RANGE_FFT_SIZE)
        
        # Range IFFT
        range_time = np.fft.ifft(padded_data, axis=1) * RANGE_FFT_SIZE
        
        # OPTIMIZATION: Crop range bins EARLY
        if max_view_range_bins < RANGE_FFT_SIZE:
             range_time_view = range_time[:, :max_view_range_bins]
        else:
             range_time_view = range_time
             
        # Update view width
        view_width = range_time_view.shape[1]

        # 2. Doppler dimension prep (Window -> Pad)
        # We pass the REDUCED view_width to the padding function
        padded_doppler = cpu_prep_doppler_fft(range_time_view, doppler_win, NUM_SYMBOLS, DOPPLER_FFT_SIZE, view_width)
        
        # Doppler FFT
        doppler_fft = np.fft.fft(padded_doppler, axis=0)
        doppler_shifted = np.fft.fftshift(doppler_fft, axes=0)
        
        # 3. Post processing (Shift -> Abs -> Log10)
        magnitude_db = cpu_calc_mag_db(doppler_fft, NUM_SYMBOLS, FFT_SIZE)
        
        return magnitude_db, range_time_view, doppler_shifted

    else:
        # 1. Range dimension processing (CPU)
        shifted_data = np.fft.fftshift(frame_data, axes=1)
        windowed_data = shifted_data * range_win
        padded_data = np.zeros((NUM_SYMBOLS, RANGE_FFT_SIZE), dtype=np.complex64)
        padded_data[:, :FFT_SIZE] = windowed_data
        range_time = np.fft.ifft(padded_data, axis=1) * RANGE_FFT_SIZE

        # OPTIMIZATION: Crop range bins EARLY
        if max_view_range_bins < RANGE_FFT_SIZE:
             range_time_view = range_time[:, :max_view_range_bins]
        else:
             range_time_view = range_time
        
        view_width = range_time_view.shape[1]

        # 2. Doppler dimension processing (CPU)
        doppler_windowed = range_time_view * doppler_win
        padded_doppler = np.zeros((DOPPLER_FFT_SIZE, view_width), dtype=np.complex64)
        padded_doppler[:NUM_SYMBOLS, :] = doppler_windowed
        doppler_fft = np.fft.fft(padded_doppler, axis=0)
        doppler_shifted = np.fft.fftshift(doppler_fft, axes=0)
        magnitude = np.abs(doppler_shifted) / np.sqrt(NUM_SYMBOLS * FFT_SIZE)
        magnitude_db = 20 * np.log10(magnitude + 1e-12)

        return magnitude_db, range_time_view, doppler_shifted


def accumulate_range_time_data(ch):
    """
    Add current frame's range_time_data to accumulation buffer
    """
    with ch.range_time_lock:
        if ch.range_time_data is not None:
            # Extract data for selected range bin
            selected_data = ch.range_time_data[:, selected_range_bin]
            
            # Add to accumulation buffer
            with ch.micro_lock:
                # Optimized: Use extend instead of loop
                ch.micro_doppler_buffer.extend(selected_data)
            
            # Trigger worker thread
            # md_event.set() # Removed in DSP refactor

def calculate_micro_doppler(ch):
    """
    Calculate the micro-Doppler spectrum using 5000 accumulated time points from the buffer
    """
    # Check buffer size
    if len(ch.micro_doppler_buffer) < 256:  # At least 256 points are required to calculate STFT
        return None, None, None
    
    # Convert buffer to NumPy array
    with ch.micro_lock:
        complex_signal = np.array(ch.micro_doppler_buffer)
    
    # Parameter settings
    fs = 1.0  # Normalized sampling rate
    nfft = 256  # FFT size
    nperseg = 256  # Points per segment
    noverlap = 192  # Overlap points
    
    # Calculate STFT
    f, t, Zxx = stft(complex_signal, fs=fs, window='hamming', 
                    nperseg=nperseg, noverlap=noverlap, nfft=nfft, 
                    return_onesided=False)
    
    # Calculate magnitude and convert to dB
    Pxx = np.abs(Zxx)
    Pxx_db = 20 * np.log10(Pxx + 1e-12)
    
    # FFT shift to move zero frequency to the center
    Pxx_db_shifted = np.fft.fftshift(Pxx_db, axes=0)
    f_shifted = np.fft.fftshift(f)
    
    # Keep zero Doppler in the center
    max_freq = 0.5  # Maximum display frequency (normalized)
    f_idx = (f_shifted > -max_freq) & (f_shifted < max_freq)
    
    return f_shifted[f_idx], t, Pxx_db_shifted[f_idx, :]

def _process_one_raw_item(ch, raw_item):
    if isinstance(raw_item, tuple) and len(raw_item) == 2:
        frame_id, raw_frame = raw_item
    else:
        frame_id, raw_frame = -1, raw_item

    t_start_dsp = time.time()

    # 1. Range-Doppler Processing (per channel)
    rd_spectrum, range_time_view, rd_complex = process_range_doppler(
        raw_frame, gpu_stream=ch.gpu_stream
    )
    with ch.range_time_lock:
        ch.range_time_data = range_time_view
    
    # Crop Display Range (same logic as before)
    if rd_spectrum.shape[1] > MAX_RANGE_BIN:
        rd_spectrum = rd_spectrum[:, :MAX_RANGE_BIN]
        rd_complex = rd_complex[:, :MAX_RANGE_BIN]
    
    # Crop Doppler Range
    center_idx = rd_spectrum.shape[0] // 2
    start_idx = max(0, center_idx - MAX_DOPPLER_BINS // 2)
    end_idx = min(rd_spectrum.shape[0], start_idx + MAX_DOPPLER_BINS)
    rd_spectrum = rd_spectrum[start_idx:end_idx, :]
    rd_complex = rd_complex[start_idx:end_idx, :]
    
    # Transpose for plotting
    rd_spectrum_plot = np.transpose(rd_spectrum)
    rd_complex_plot = np.transpose(rd_complex).astype(np.complex64, copy=False)
    
    # OPTIMIZATION: Downsample for display to speed up set_data/rendering
    if DISPLAY_DOWNSAMPLE > 1:
        rd_spectrum_plot = rd_spectrum_plot[::DISPLAY_DOWNSAMPLE, ::DISPLAY_DOWNSAMPLE]
        rd_complex_plot = rd_complex_plot[::DISPLAY_DOWNSAMPLE, ::DISPLAY_DOWNSAMPLE]

    # 2. Micro-Doppler Processing
    md_spectrum = None
    md_extent = None
    
    if show_micro_doppler:
        # Accumulate data
        accumulate_range_time_data(ch)
        f, t, Pxx = calculate_micro_doppler(ch)
        if Pxx is not None:
            md_spectrum = Pxx
            md_extent = [t[0], t[-1], f[0], f[-1]]
    
    t_end_dsp = time.time()
    dsp_time = t_end_dsp - t_start_dsp
    
    # 3. Push to Display Queue
    result = {
        'ch_id': ch.ch_id,
        'frame_id': int(frame_id),
        'raw': raw_frame,
        'rd': rd_spectrum_plot,
        'rd_complex': rd_complex_plot,
        'md': md_spectrum,
        'md_extent': md_extent,
        'dsp_time': dsp_time
    }
    
    if ch.display_queue.full():
        try:
            ch.display_queue.get_nowait() # Drop oldest display frame to keep latency low
        except:
            pass
    ch.display_queue.put(result)


def dsp_worker(ch):
    """
    Per-channel DSP worker (used for CPU path / single-channel GPU path).
    """
    print(f"[CH{ch.ch_id}] DSP Worker started")
    while True:
        try:
            raw_item = ch.raw_frame_queue.get(timeout=0.1)
        except Exception:
            continue

        try:
            _process_one_raw_item(ch, raw_item)
        except Exception as e:
            print(f"[CH{ch.ch_id}] DSP Worker Error: {e}")
            import traceback
            traceback.print_exc()


def dsp_worker_single_gpu():
    """
    Single GPU worker thread:
    - one host thread submits all channel jobs (no global lock),
    - each channel uses its own CUDA stream.
    """
    print("[GPU] Single GPU worker started (per-channel streams).")
    rr_idx = 0
    n_ch = len(CHANNELS)

    while True:
        did_work = False
        for step in range(n_ch):
            ch = CHANNELS[(rr_idx + step) % n_ch]
            latest_item = None
            while True:
                try:
                    latest_item = ch.raw_frame_queue.get_nowait()
                except Empty:
                    break
                except Exception:
                    break

            if latest_item is None:
                continue

            try:
                _process_one_raw_item(ch, latest_item)
                did_work = True
            except Exception as e:
                print(f"[CH{ch.ch_id}] Single GPU worker error: {e}")
                import traceback
                traceback.print_exc()

        if n_ch > 0:
            rr_idx = (rr_idx + 1) % n_ch
        if not did_work:
            time.sleep(0.001)

# ====== Control Command Functions ======
def send_alignment_command(adjustment):
    # Set target channel in modulator, then send ALGN value.
    send_control_command(b"ALCH", int(active_channel), active_channel)
    send_control_command(b"ALGN", int(adjustment), active_channel)

def send_strd_command(val):
    """Send STRD configuration command"""
    try:
        strd_val = int(val)
        # Ensure value is within reasonable range
        if strd_val < 1: strd_val = 1
        if strd_val > NUM_SYMBOLS: strd_val = NUM_SYMBOLS
        
        send_control_command(b"STRD", int(strd_val), None)
    except ValueError:
        print(f"Invalid STRD value: {val}")

def send_mti_command(enabled):
    """Send MTI control command to C++"""
    send_control_command(b"MTI ", 1 if enabled else 0, None)

def update_range_bin(val):
    global selected_range_bin, phase_probe_range
    selected_range_bin = int(val)
    phase_probe_range = float(selected_range_bin)
    
    # Clear accumulation buffer for all channels when changing range bin
    for ch in CHANNELS:
        with ch.micro_lock:
            ch.micro_doppler_buffer.clear()
    
    print(f"Selected range bin updated to {selected_range_bin}, all channel buffers cleared")

def toggle_micro_doppler(event):
    global show_micro_doppler
    show_micro_doppler = not show_micro_doppler
    btn_md.color = 'lightgreen' if show_micro_doppler else 'lightgray'
    btn_md.ax.set_facecolor(btn_md.color)
    print(f"Micro-Doppler display {'enabled' if show_micro_doppler else 'disabled'}")

# ====== GUI Setup ======
print("Setting up visualization...")
if len(CHANNELS) < 2:
    print("Warning: Fewer than 2 channels configured; CH1 plots will remain blank.")

display_channel = 0 if len(CHANNELS) > 0 else -1
current_tab = "rdmd"
phase_probe_range = float(selected_range_bin)
phase_probe_doppler = 0.0
phase_probe_valid = False

fig = plt.figure(figsize=(22, 12))
ax_rd = fig.add_axes([0.06, 0.56, 0.50, 0.39])
ax_md = fig.add_axes([0.06, 0.08, 0.50, 0.39])
ax_phase = fig.add_axes([0.06, 0.08, 0.50, 0.87])

# Initial empty image
init_data_rd = np.zeros((MAX_RANGE_BIN, MAX_DOPPLER_BINS), dtype=np.float32)
init_data_md = np.zeros((64, 100), dtype=np.float32)

doppler_bins = np.linspace(-0.5, 0.5, MAX_DOPPLER_BINS)
rd_extent = [doppler_bins[0], doppler_bins[-1], 0, MAX_RANGE_BIN]

img_rd = ax_rd.imshow(
    init_data_rd,
    aspect='auto',
    origin='lower',
    extent=rd_extent,
    cmap=cm.jet,
    interpolation='nearest',
    vmin=0,
    vmax=60,
)
ax_rd.set_xlabel('Doppler Frequency')
ax_rd.set_ylabel('Range Bin')
ax_rd.set_title('Range-Doppler Spectrum')

rd_rect = patches.Rectangle(
    (doppler_bins[0], selected_range_bin),
    doppler_bins[-1] - doppler_bins[0],
    3,
    linewidth=1,
    edgecolor='r',
    facecolor='r',
    alpha=0.25,
)
ax_rd.add_patch(rd_rect)

sender_text = ax_rd.text(
    0.02, 0.95, "Sender: Detecting...",
    transform=ax_rd.transAxes,
    color='white', ha='left', va='top', fontsize=9,
    bbox=dict(facecolor='black', alpha=0.5)
)

phase_probe_text = ax_rd.text(
    0.02, 0.86, "Phase@clicked point: waiting for selection",
    transform=ax_rd.transAxes,
    color='white', ha='left', va='top', fontsize=9,
    bbox=dict(facecolor='black', alpha=0.5)
)

phase_probe_marker, = ax_rd.plot(
    [], [], marker='x', markersize=11, markeredgewidth=2.0,
    color='white', linestyle='None'
)

img_md = ax_md.imshow(
    init_data_md,
    aspect='auto',
    origin='lower',
    cmap=cm.jet,
    interpolation='nearest',
    vmin=0,
    vmax=60,
)
ax_md.set_xlabel('Time (Sample Index)')
ax_md.set_ylabel('Doppler Frequency')
ax_md.set_title('Micro-Doppler Spectrum')

buffer_text = ax_md.text(
    0.02, 0.95, "Buffer: 0/5000",
    transform=ax_md.transAxes,
    color='white', ha='left', va='top', fontsize=9,
    bbox=dict(facecolor='black', alpha=0.5)
)

img_phase = ax_phase.imshow(
    np.zeros_like(init_data_rd),
    aspect='auto',
    origin='lower',
    extent=rd_extent,
    cmap=cm.jet,  # Keep same color scheme family as RD
    interpolation='nearest',
    vmin=-np.pi,
    vmax=np.pi,
)
ax_phase.set_xlabel('Doppler Frequency')
ax_phase.set_ylabel('Range Bin')
ax_phase.set_title('Phase Difference Spectrum (CH2 - CH1, Same Frame)')
phase_tab_text = ax_phase.text(
    0.02, 0.97, "Phase sync: waiting CH1/CH2 same-frame data",
    transform=ax_phase.transAxes,
    color='white', ha='left', va='top', fontsize=10,
    bbox=dict(facecolor='black', alpha=0.55)
)

# Phase sync cache for same-frame comparison
PHASE_SYNC_CACHE_SIZE = 128
phase_sync_cache = [dict(), dict()]  # slot 0/1: frame_id -> rd_complex
phase_sync_order = [deque(maxlen=PHASE_SYNC_CACHE_SIZE), deque(maxlen=PHASE_SYNC_CACHE_SIZE)]

def cache_phase_frame(slot, frame_id, rd_complex):
    if slot not in (0, 1) or frame_id is None or frame_id < 0 or rd_complex is None:
        return
    cache = phase_sync_cache[slot]
    order = phase_sync_order[slot]
    if frame_id in cache:
        cache[frame_id] = rd_complex
        return
    if len(order) == order.maxlen:
        old_id = order.popleft()
        cache.pop(old_id, None)
    order.append(frame_id)
    cache[frame_id] = rd_complex

# Add colorbars
cax_rd = fig.add_axes([0.57, 0.56, 0.012, 0.39])
cax_md = fig.add_axes([0.57, 0.08, 0.012, 0.39])
cax_phase = fig.add_axes([0.57, 0.08, 0.012, 0.87])

cbar_rd = fig.colorbar(img_rd, cax=cax_rd)
cbar_rd.set_label('RD Magnitude (dB)')
cbar_md = fig.colorbar(img_md, cax=cax_md)
cbar_md.set_label('MD Magnitude (dB)')
cbar_phase = fig.colorbar(img_phase, cax=cax_phase)
cbar_phase.set_label('Phase Diff (rad)')

# Control panel (right side)
ax_tab_rdmd = plt.axes([0.63, 0.90, 0.15, 0.04])
btn_tab_rdmd = Button(ax_tab_rdmd, 'RD+MicroDoppler')
ax_tab_phase = plt.axes([0.80, 0.90, 0.15, 0.04])
btn_tab_phase = Button(ax_tab_phase, 'Phase Diff')

ax_disp_ch1 = plt.axes([0.63, 0.84, 0.15, 0.04])
btn_disp_ch1 = Button(ax_disp_ch1, 'Show CH1')
ax_disp_ch2 = plt.axes([0.80, 0.84, 0.15, 0.04])
btn_disp_ch2 = Button(ax_disp_ch2, 'Show CH2')

# Range bin selector
ax_range_textbox = plt.axes([0.70, 0.78, 0.12, 0.035])
range_text_box = TextBox(ax_range_textbox, 'Range Bin:', initial=str(selected_range_bin))

ax_range_submit = plt.axes([0.84, 0.78, 0.07, 0.035])
range_submit_btn = Button(ax_range_submit, 'Set')
range_submit_btn.label.set_fontsize(8)

def set_range_bin(text):
    try:
        bin_val = int(text)
        # Ensure value is within reasonable range
        if bin_val < 0: bin_val = 0
        if bin_val >= MAX_RANGE_BIN: bin_val = MAX_RANGE_BIN - 1
        
        update_range_bin(bin_val)
        print(f"Range bin set to: {bin_val}")
    except ValueError:
        print(f"Invalid range bin value: {text}")

range_submit_btn.on_clicked(lambda event: set_range_bin(range_text_box.text))

# Micro-Doppler toggle
ax_md_btn = plt.axes([0.63, 0.73, 0.10, 0.035])
btn_md = Button(ax_md_btn, 'Micro-Doppler', color='lightgreen')
btn_md.on_clicked(toggle_micro_doppler)

# MTI button
ax_mti_btn = plt.axes([0.74, 0.73, 0.09, 0.035])
btn_mti = Button(ax_mti_btn, 'MTI', color='lightgreen' if enable_mti else 'lightgray')

def toggle_mti(event):
    global enable_mti
    enable_mti = not enable_mti
    btn_mti.color = 'lightgreen' if enable_mti else 'lightgray'
    btn_mti.ax.set_facecolor(btn_mti.color)
    print(f"MTI {'enabled' if enable_mti else 'disabled'}")
    send_mti_command(enable_mti)
    
btn_mti.on_clicked(toggle_mti)

# Range window toggle button
ax_range_window_btn = plt.axes([0.84, 0.73, 0.11, 0.035])
btn_range_window = Button(ax_range_window_btn, 'Range Win', color='lightgreen' if enable_range_window else 'lightgray')

def toggle_range_window(event):
    global enable_range_window
    enable_range_window = not enable_range_window
    btn_range_window.color = 'lightgreen' if enable_range_window else 'lightgray'
    btn_range_window.ax.set_facecolor(btn_range_window.color)
    print(f"Range windowing {'enabled' if enable_range_window else 'disabled'}")
    
btn_range_window.on_clicked(toggle_range_window)

# Doppler window toggle button
ax_doppler_window_btn = plt.axes([0.63, 0.68, 0.15, 0.035])
btn_doppler_window = Button(ax_doppler_window_btn, 'Doppler Win', color='lightgreen' if enable_doppler_window else 'lightgray')

def toggle_doppler_window(event):
    global enable_doppler_window
    enable_doppler_window = not enable_doppler_window
    btn_doppler_window.color = 'lightgreen' if enable_doppler_window else 'lightgray'
    btn_doppler_window.ax.set_facecolor(btn_doppler_window.color)
    print(f"Doppler windowing {'enabled' if enable_doppler_window else 'disabled'}")
    
btn_doppler_window.on_clicked(toggle_doppler_window)

# Alignment control
ax_textbox = plt.axes([0.70, 0.62, 0.12, 0.035])
text_box = TextBox(ax_textbox, 'Delay:', initial="0")
alignment_value = 0

ax_submit = plt.axes([0.84, 0.62, 0.07, 0.035])
submit_btn = Button(ax_submit, 'Apply')
submit_btn.label.set_fontsize(8)

def apply_alignment(event):
    global alignment_value
    try:
        val = text_box.text.strip()
        if val:
            alignment_value = int(val)
        send_alignment_command(alignment_value)
        print(f"Sent alignment command: {alignment_value} samples")
    except ValueError:
        print(f"Invalid alignment value: {val}")
submit_btn.on_clicked(apply_alignment)

# Quick adjustment buttons
ax_btn_plus10 = plt.axes([0.63, 0.57, 0.07, 0.035])
btn_plus10 = Button(ax_btn_plus10, '+10')
btn_plus10.on_clicked(lambda event: send_alignment_command(10))

ax_btn_minus10 = plt.axes([0.71, 0.57, 0.07, 0.035])
btn_minus10 = Button(ax_btn_minus10, '-10')
btn_minus10.on_clicked(lambda event: send_alignment_command(-10))

ax_btn_plus = plt.axes([0.79, 0.57, 0.07, 0.035])
btn_plus = Button(ax_btn_plus, '+1')
btn_plus.on_clicked(lambda event: send_alignment_command(1))

ax_btn_minus = plt.axes([0.87, 0.57, 0.07, 0.035])
btn_minus = Button(ax_btn_minus, '-1')
btn_minus.on_clicked(lambda event: send_alignment_command(-1))

# STRD control
ax_strd_textbox = plt.axes([0.70, 0.52, 0.12, 0.035])
strd_text_box = TextBox(ax_strd_textbox, 'STRD:', initial="20")

ax_strd_submit = plt.axes([0.84, 0.52, 0.07, 0.035])
strd_submit_btn = Button(ax_strd_submit, 'Set')
strd_submit_btn.label.set_fontsize(8)
strd_submit_btn.on_clicked(lambda event: send_strd_command(strd_text_box.text))
btn_md.ax.set_facecolor(btn_md.color)
btn_mti.ax.set_facecolor(btn_mti.color)
btn_range_window.ax.set_facecolor(btn_range_window.color)
btn_doppler_window.ax.set_facecolor(btn_doppler_window.color)

sparse_text = fig.text(
    0.79, 0.22,
    "STRD: Sparse Sampling Stride for OFDM Symbols",
    ha='center', va='bottom', fontsize=9, color='gray'
)
info_text = fig.text(
    0.79, 0.19,
    "Set delay then click 'Apply' or use quick buttons",
    ha='center', va='bottom', fontsize=9, color='gray'
)

# ====== Save Functions ======
def save_raw_frame(event):
    ch = CHANNELS[active_channel]
    if 'raw' in ch.current_display_data:
        data = ch.current_display_data['raw']
        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        fname = f"./capture/capture_raw_ch{ch.ch_id}_{ts}.mat"
        try:
            os.makedirs("./capture", exist_ok=True)
            sio.savemat(fname, {'raw_frame': data})
            print(f"Saved Raw frame to {fname} (Shape: {data.shape})")
        except Exception as e:
            print(f"Error saving Raw frame: {e}")
    else:
        print("No Raw data available to save")

def save_rd_map(event):
    ch = CHANNELS[active_channel]
    if 'rd' in ch.current_display_data:
        data = ch.current_display_data['rd']
        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        fname = f"./capture/capture_rd_ch{ch.ch_id}_{ts}.mat"
        try:
            os.makedirs("./capture", exist_ok=True)
            sio.savemat(fname, {'rd_map': data})
            print(f"Saved RD Map to {fname} (Shape: {data.shape})")
        except Exception as e:
            print(f"Error saving RD Map: {e}")
    else:
        print("No RD Map data available to save")

def save_md_map(event):
    ch = CHANNELS[active_channel]
    if 'md' in ch.current_display_data:
        data = ch.current_display_data['md']
        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        fname = f"./capture/capture_md_ch{ch.ch_id}_{ts}.mat"
        try:
            os.makedirs("./capture", exist_ok=True)
            sio.savemat(fname, {'md_map': data})
            print(f"Saved Micro-Doppler Map to {fname} (Shape: {data.shape})")
        except Exception as e:
            print(f"Error saving MD Map: {e}")
    else:
        print("No MD Map data available to save")

# Save Buttons
btn_width = 0.08
btn_height = 0.035
btn_y = 0.46

ax_save_raw = plt.axes([0.63, btn_y, 0.10, btn_height])
btn_save_raw = Button(ax_save_raw, 'Save Raw')
btn_save_raw.label.set_fontsize(8)
btn_save_raw.on_clicked(save_raw_frame)

ax_save_rd = plt.axes([0.74, btn_y, 0.10, btn_height])
btn_save_rd = Button(ax_save_rd, 'Save RD')
btn_save_rd.label.set_fontsize(8)
btn_save_rd.on_clicked(save_rd_map)

ax_save_md = plt.axes([0.85, btn_y, 0.10, btn_height])
btn_save_md = Button(ax_save_md, 'Save MD')
btn_save_md.label.set_fontsize(8)
btn_save_md.on_clicked(save_md_map)

# Status display below the button array
ax_status_panel = fig.add_axes([0.63, 0.27, 0.32, 0.16])
ax_status_panel.axis("off")
fps_text = ax_status_panel.text(
    0.00, 0.98, "FPS: 0.0",
    transform=ax_status_panel.transAxes,
    ha='left', va='top', fontsize=10, color='black',
    bbox=dict(facecolor='whitesmoke', alpha=0.9, edgecolor='lightgray')
)
queue_text = ax_status_panel.text(
    0.00, 0.45, "Queue: 0",
    transform=ax_status_panel.transAxes,
    ha='left', va='top', fontsize=10, color='black',
    bbox=dict(facecolor='whitesmoke', alpha=0.9, edgecolor='lightgray')
)
control_text = ax_status_panel.text(
    0.00, 0.08, "Display CH: 1 | ALGN target CH: 1",
    transform=ax_status_panel.transAxes,
    ha='left', va='top', fontsize=10, color='black',
    bbox=dict(facecolor='whitesmoke', alpha=0.9, edgecolor='lightgray')
)

def _set_button_state(btn, is_on, on_color='lightgreen', off_color='lightgray'):
    btn.color = on_color if is_on else off_color
    btn.ax.set_facecolor(btn.color)

def _refresh_channel_titles():
    if 0 <= display_channel < len(CHANNELS):
        ch = CHANNELS[display_channel]
        ch_label = f"CH{ch.ch_id + 1}"
        active_label = f"CH{active_channel + 1}" if 0 <= active_channel < len(CHANNELS) else "N/A"
    else:
        ch_label = "N/A"
        active_label = "N/A"
    ax_rd.set_title('Range-Doppler Spectrum')
    ax_md.set_title('Micro-Doppler Spectrum')
    control_text.set_text(f"Display CH: {ch_label} | ALGN target CH: {active_label}")

def set_display_channel(ch_idx):
    global display_channel, active_channel
    if not CHANNELS:
        return
    max_supported = min(len(CHANNELS), 2) - 1
    ch_idx = max(0, min(ch_idx, max_supported))
    display_channel = ch_idx
    active_channel = ch_idx
    _set_button_state(btn_disp_ch1, display_channel == 0, on_color='lightskyblue')
    _set_button_state(btn_disp_ch2, display_channel == 1, on_color='lightskyblue')
    _refresh_channel_titles()
    print(f"Display channel switched to CH{display_channel + 1}")

def set_tab(tab_name):
    global current_tab
    if tab_name not in ("rdmd", "phase"):
        return
    current_tab = tab_name
    show_rdmd = (tab_name == "rdmd")
    ax_rd.set_visible(show_rdmd)
    ax_md.set_visible(show_rdmd)
    cax_rd.set_visible(show_rdmd)
    cax_md.set_visible(show_rdmd)
    img_rd.set_visible(show_rdmd)
    img_md.set_visible(show_rdmd)
    rd_rect.set_visible(show_rdmd)
    sender_text.set_visible(show_rdmd)
    buffer_text.set_visible(show_rdmd)
    phase_probe_text.set_visible(show_rdmd)
    phase_probe_marker.set_visible(show_rdmd)
    ax_phase.set_visible(not show_rdmd)
    cax_phase.set_visible(not show_rdmd)
    img_phase.set_visible(not show_rdmd)
    phase_tab_text.set_visible(not show_rdmd)
    _set_button_state(btn_tab_rdmd, show_rdmd, on_color='lightskyblue')
    _set_button_state(btn_tab_phase, not show_rdmd, on_color='lightskyblue')
    fig.canvas.draw_idle()

def on_rd_click(event):
    global phase_probe_range, phase_probe_doppler, phase_probe_valid
    if event.inaxes != ax_rd or event.xdata is None or event.ydata is None:
        return
    doppler_val = float(np.clip(event.xdata, rd_extent[0], rd_extent[1]))
    range_val = float(np.clip(event.ydata, 0, MAX_RANGE_BIN - 1))
    phase_probe_doppler = doppler_val
    phase_probe_range = range_val
    phase_probe_valid = True
    phase_probe_marker.set_data([phase_probe_doppler], [phase_probe_range])
    fig.canvas.draw_idle()

btn_tab_rdmd.on_clicked(lambda event: set_tab("rdmd"))
btn_tab_phase.on_clicked(lambda event: set_tab("phase"))
btn_disp_ch1.on_clicked(lambda event: set_display_channel(0))
btn_disp_ch2.on_clicked(lambda event: set_display_channel(1))
fig.canvas.mpl_connect('button_press_event', on_rd_click)

set_display_channel(display_channel if display_channel >= 0 else 0)
set_tab("rdmd")

# Send SKIP command
send_skip_command()

# ====== Animation Update Function ======
last_update_time = time.time()
frame_count = 0
total_dsp_time = 0.0  # Accumulate DSP processing time
total_python_time = 0.0 # Accumulate Total processing time
fps_update_interval = 0.5  # FPS update interval (seconds)

def update(frame_num):
    global last_update_time, frame_count, running, selected_range_bin, total_dsp_time, total_python_time
    
    t_start_update = time.time() # Start Total Timer
    
    if not running:
        return []
    
    try:
        # Calculate FPS
        current_time = time.time()
        elapsed = current_time - last_update_time
        
        # Update FPS display
        if elapsed > fps_update_interval:
            fps = frame_count / elapsed
            avg_dsp = (total_dsp_time / frame_count * 1000) if frame_count > 0 else 0
            avg_total = (total_python_time / frame_count * 1000) if frame_count > 0 else 0
            
            fps_text.set_text(f"FPS: {fps:.1f}\nDSP: {avg_dsp:.1f}ms\nTotal: {avg_total:.1f}ms")
            
            frame_count = 0
            total_dsp_time = 0.0
            total_python_time = 0.0
            last_update_time = current_time
        
        # Update queue status
        queue_states = " ".join(
            [f"C{i}:{CHANNELS[i].raw_frame_queue.qsize()}/{CHANNELS[i].display_queue.qsize()}" for i in range(len(CHANNELS))]
        )
        queue_text.set_text(f"Q(raw/disp) {queue_states}")
        
        updated_any = False

        # Pull latest processed frame for each channel
        for slot, ch in enumerate(CHANNELS):
            latest_result = None
            while not ch.display_queue.empty():
                latest_result = ch.display_queue.get()

            if latest_result is not None:
                ch.current_display_data = latest_result
                ch.last_frame_id = latest_result.get('frame_id', ch.last_frame_id)
                if slot < 2:
                    cache_phase_frame(slot, latest_result.get('frame_id', -1), latest_result.get('rd_complex'))

                if 'dsp_time' in latest_result:
                    total_dsp_time += latest_result['dsp_time']

                updated_any = True

        # Update displayed channel (single-channel RD/MD view)
        if 0 <= display_channel < len(CHANNELS):
            disp_ch = CHANNELS[display_channel]
            latest_disp = disp_ch.current_display_data

            if 'rd' in latest_disp:
                img_rd.set_data(latest_disp['rd'])
            if latest_disp.get('md') is not None:
                img_md.set_data(latest_disp['md'])
                if latest_disp.get('md_extent'):
                    img_md.set_extent(latest_disp['md_extent'])

            with disp_ch.sender_lock:
                if disp_ch.sender_ip:
                    sender_text.set_text(
                        f"Sender: {disp_ch.sender_ip}:{disp_ch.control_port} | Frame: {disp_ch.last_frame_id}"
                    )
                else:
                    sender_text.set_text("Sender: Detecting...")

            with disp_ch.micro_lock:
                buffer_text.set_text(f"Buffer(CH{disp_ch.ch_id + 1}): {len(disp_ch.micro_doppler_buffer)}/{BUFFER_LENGTH}")
        else:
            sender_text.set_text("Sender: N/A")
            buffer_text.set_text("Buffer: N/A")

        rd_rect.set_y(selected_range_bin)
        disp_label = f"CH{display_channel + 1}" if 0 <= display_channel < len(CHANNELS) else "N/A"
        act_label = f"CH{active_channel + 1}" if 0 <= active_channel < len(CHANNELS) else "N/A"
        control_text.set_text(f"Display CH: {disp_label} | ALGN target CH: {act_label}")

        # CH1-CH0 phase difference spectrum, strictly on the same frame_id
        phase_ready = False
        phase_diff = None
        synced_frame_id = None
        if len(CHANNELS) >= 2:
            common_frame_ids = set(phase_sync_cache[0].keys()).intersection(phase_sync_cache[1].keys())
            if common_frame_ids:
                synced_frame_id = max(common_frame_ids)
                rd0 = phase_sync_cache[0].get(synced_frame_id)
                rd1 = phase_sync_cache[1].get(synced_frame_id)
                if rd0 is not None and rd1 is not None and rd0.shape == rd1.shape:
                    phase_diff = np.angle(rd1 * np.conj(rd0))
                    img_phase.set_data(phase_diff)
                    phase_ready = True

                # Drop very old synchronized data from cache
                for cache_slot in (0, 1):
                    cache = phase_sync_cache[cache_slot]
                    order = phase_sync_order[cache_slot]
                    while order and order[0] < (synced_frame_id - 8):
                        old_id = order.popleft()
                        cache.pop(old_id, None)

        if phase_ready:
            phase_tab_text.set_text("Phase sync: locked (CH2 - CH1)")
        else:
            ch0_fid = CHANNELS[0].last_frame_id if len(CHANNELS) > 0 else None
            ch1_fid = CHANNELS[1].last_frame_id if len(CHANNELS) > 1 else None
            phase_tab_text.set_text(f"Phase sync: waiting same frame (CH1={ch0_fid}, CH2={ch1_fid})")

        # RD tab: phase difference numeric readout at clicked RD point
        if not phase_probe_valid:
            phase_probe_text.set_text("Phase@clicked point: click RD map to query")
            phase_probe_marker.set_data([], [])
        elif phase_ready and phase_diff is not None and phase_diff.size > 0:
            phase_probe_marker.set_data([phase_probe_doppler], [phase_probe_range])

            rows, cols = phase_diff.shape
            range_norm = phase_probe_range / max(1.0, float(MAX_RANGE_BIN - 1))
            doppler_norm = (phase_probe_doppler - rd_extent[0]) / max(1e-9, (rd_extent[1] - rd_extent[0]))
            r_idx = int(np.clip(round(range_norm * (rows - 1)), 0, rows - 1))
            c_idx = int(np.clip(round(doppler_norm * (cols - 1)), 0, cols - 1))

            ph_val = float(phase_diff[r_idx, c_idx])
            phase_probe_text.set_text(
                f"Phase@R={phase_probe_range:.1f}, D={phase_probe_doppler:+.3f}: {ph_val:+.3f} rad"
            )
        else:
            phase_probe_marker.set_data([phase_probe_doppler], [phase_probe_range])
            phase_probe_text.set_text("Phase@clicked point: waiting CH1/CH2 same-frame data")

        if updated_any:
            frame_count += 1
        
        # Return objects to update
        t_end_update = time.time()
        total_python_time += (t_end_update - t_start_update)
        return [
            img_rd, img_md, img_phase,
            rd_rect, sender_text, buffer_text,
            phase_probe_text, phase_probe_marker, phase_tab_text,
            fps_text, queue_text, control_text
        ]
    except Exception as e:
        print(f"Update error: {str(e)}")
        import traceback
        traceback.print_exc()
        return []

# ====== Close Event Handling ======
def on_close(event):
    global running
    print("Closing visualization...")
    running = False  # Stop animation loop
    print("Visualization closed. Receiver/DSP threads will exit automatically.")

# Connect close event
fig.canvas.mpl_connect('close_event', on_close)

# ====== Start Animation ======
print("Starting animation...")

# Start DSP Worker Threads (one per channel)
dsp_threads = []
if USE_NVIDIA_GPU and len(CHANNELS) > 1:
    _dt = threading.Thread(target=dsp_worker_single_gpu, daemon=True)
    _dt.start()
    dsp_threads.append(_dt)
else:
    for _ch in CHANNELS:
        _dt = threading.Thread(target=dsp_worker, args=(_ch,), daemon=True)
        _dt.start()
        dsp_threads.append(_dt)

try:
    # Use short interval for high refresh rate
    ani = FuncAnimation(fig, update, 
                        interval=1,  # Try to update as fast as possible (was 50)
                        blit=True,
                        save_count=50)
    
    # Robust GUI loop: some environments may return from show() unexpectedly.
    plt.show(block=False)
    while running and plt.fignum_exists(fig.number):
        plt.pause(0.1)
except Exception as e:
    print(f"Animation error: {str(e)}")
finally:
    running = False
    print("Animation stopped.")
