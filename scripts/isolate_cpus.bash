...
if [ "$1" == "run" ]; then
    ...
    # Preserve config-related environment for applications that rely on per-user
    # config discovery (e.g. UHD looks in HOME/XDG_CONFIG_HOME unless a system
    # config or UHD_CONFIG_FILE is provided).
    RUN_ENV_ARGS=()
    if [ -n "$HOME" ]; then
        RUN_ENV_ARGS+=(--setenv=HOME="$HOME")
    fi
    if [ -n "$XDG_CONFIG_HOME" ]; then
        RUN_ENV_ARGS+=(--setenv=XDG_CONFIG_HOME="$XDG_CONFIG_HOME")
    elif [ -n "$HOME" ]; then
        RUN_ENV_ARGS+=(--setenv=XDG_CONFIG_HOME="$HOME/.config")
    fi
    if [ -n "$UHD_CONFIG_FILE" ]; then
        RUN_ENV_ARGS+=(--setenv=UHD_CONFIG_FILE="$UHD_CONFIG_FILE")
    fi
    if [ -n "$LD_LIBRARY_PATH" ]; then
        RUN_ENV_ARGS+=(--setenv=LD_LIBRARY_PATH="$LD_LIBRARY_PATH")
    fi
    ...
    systemd-run --slice=rt-workload.slice --pty -p AllowedCPUs=$RUN_CPUS \
        -p WorkingDirectory="$(pwd)" "${RUN_ENV_ARGS[@]}" "$@"
    exit $?
fi
...
echo ""
echo "  sudo ./scripts/isolate_cpus.bash run ./your_application"
echo ""
